...
namespace wojilu.Data
{
    ...
    public class DbConfig
    {
        ...
        public static void checkConnectionString(DbConfig result)
        {
        ...
        public static String GetConnectionString(String db)
        {
            if (db == "default")
            {
                object CompanyCode = HttpContext.Current.Session["CompanyCode"];
                if (CompanyCode != null && strUtil.HasText(CompanyCode.ToString()))
                {
                    db = CompanyCode.ToString();

                }
            }
            if (DbConfig.Instance.ConnectionStringTable.ContainsKey(db) == false)
            {
                DbConfig.Instance.ConnectionStringTable.Add(db, (object)string.Format(DbConfig.Instance.ConnectionStringTable[DbConfig.DefaultDbName].ToString(), db));
                checkConnectionString(DbConfig.Instance);
                SaveConnectionString();
                DbConfig.Instance.DbType.Add(db, ((object)"sqlserver"));
